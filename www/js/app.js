        var appKey    = "2fa96041bc635aa212227608bf88e9a30f62b5c19d3f7b1496204ede0b388b68";	//kozakura appKey
        var clientKey = "a0017bf78a668c5b81e816d49d53b4a4f1a06ad17dc2d05a5f72b7ace605bb2e"; //kozakura clientKey
        var imgsrc = "https://mb.api.cloud.nifty.com/2013-09-01/applications/GmEYOCKn0H1C9qQ6/publicFiles/"; //kozakura imgurl
        var projectno = "432306700091"; //プロジェクトkey
        var appname = "1.0.0(L307)";
        var mailsave = "http://ideaxidea.net/phpmail/kozakura.php?title=";
        var mode = "ビルド";//ビルド or "debug"　にすると携帯端末限定処理を飛ばしてPC側でも操作確認ができるようになります
		var en_point  = ["小桜保育園",new google.maps.LatLng(33.30038,130.38026)];	//小桜
		var class_datalist = ["つぼみ","チューリップ","ひまわり","もも・ばら・ゆり","さくら・うめ"];
		function homepage() {var ref = window.open('http://kozakurahoikuen.net/', '_blank');}	//kozakura
...
function onRegisterBtn2() {
    ...
    //入力チェック
	  ...
	  if(telphone == ""){
		$('#regi3').html('<span style="color:#EA4335">電話番号は必須です</span>');
		$('#tourokumemo').html('<h5>入力項目を再度ご確認ください</h5>');
		er = 1;
	  }
	if(checgroup == ""){
	  ...
	  //電話番号書式チェック
	var hyphen = telphone.replace(/[━.*‐.*―.*－.*\?.*ー.*\-]/gi,'');
	if(!hyphen.match(/^[0-9]+$/)){
		ons.notification.alert({title:telphone,message:"電話番号の表記が正しくありません。"});
		$('#regi3').append('<span style="color:#EA4335">電話番号の表記が正しくありません</span>');
		$('#tourokumenu').show();
		$('.load').hide();
		$('#load_txt').html('ロード中');
		return false;
	  ...
	  //電話番号重複チェック
		ncmb.User.where({Tel: hyphen})
			.limit(1000)
			.fetch()
			.then(function(test) {
			if (test.objectId){
				ons.notification.alert({title:telphone,message:"この電話番号は既に使用されています。"});
				$('#regi3').append('<span style="color:#EA4335">この電話番号は既に使用されています</span>');
				$('#tourokumenu').show();
				$('.load').hide();
				$('#load_txt').html('ロード中');
				return;
    ...
    user.signUpByAccount()
        ...
        .catch(function(error) {
         ...
         .catch(function(err){
			ons.notification.alert({title:group,message:"電話番号重複チェック中にエラー" + err});
		});
				//ユーザ重複チェック
			}
		})
	...
	localStorage.setItem('totyu', 0);
		...
		for(for_c = 0;for_c < class_datalist.length ;for_c++){
			if($.inArray(class_datalist[for_c], userclass) >= 0){
				$('#hogosya_list_'+class_datalist[for_c]).prepend('<dl class="cf contact_btn" id="'+nameID+'" tel="'+Tel+'"><dt>'+userName+'</dt></dl>');
			}
		}
		}
...
function contactpage() {
  $('.contact_btn').on('click', function(event) {
    ...
    var tel = $(this).attr("tel");	//リンク元のidを読み込み
    if ("11" == String(tel).length){
    	denwa = insertStr(String(tel), 3, '_');
    	denwa = insertStr(denwa, 8, '_');
    	};
  //NCMB.initialize(appKey, clientKey);
	...
	ncmb.User.limit(1000)
         ...
         .then(function(results){
            for (var i = 0; i < results.length; i++) {
              var object = results[i];
                title_name = (object.get("userName"));
                objectId = (object.get("objectId"));
	             if(objectId == id){
	if( userclass == "管理者" ){
		$('.talk_user').html(title_name);
	}
    				//終了タグは最後に
    				
    // 確認ダイアログの表示
			if(tel == undefined){confirm_menu =['いいえ','はい']}else{confirm_menu =['音声通話', 'トーク','キャンセル']}
			if(tel == undefined){confirm_msg =title_name+'さんのトーク画面を開きますか？'}else{confirm_msg = title_name+'さんへ連絡をしますか？'}
		ons.notification.confirm({
			title: '',
			messageHTML: confirm_msg,
			buttonLabels: confirm_menu,
			animation: 'default',
			cancelable: true,
			callback: function(index) {
				if(index == -1) {
		...
		$('#contact_list').append('<li class="cf"><div class="e_area"><div class="e_message">'+message+'<div class="e_time">'+messagetime+'</div></div><div class="e_name">'+name+'</div></div></li>');
	            ...
	            if(no_contact !== "no"){
	            		$('#contact_list').append('<li class="cf"><div class="e_area"><div class="e_message">メッセージを送信すると、園にお問い合わせや連絡をすることができます。</div>'+
	            									'<div class="e_name">保育園</div></div></li>');
	            }
	 //下にスクロール
	 contact_delete();
        ons.notification.confirm({
