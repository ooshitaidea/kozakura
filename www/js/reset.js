...
function myhelp_tel() {
    ...
    my_tel = $("#my_tel").val();
    if (my_tel == ""){
			$('#er3').html('<span style="color:#EA4335">電話番号が入力されていません。</span>');
    			ons.notification.alert({title:"エラー",message: '電話番号が入力されていません。'});
			$('#my3').html('<div class="blue_btn" onclick="myhelp_tel()">アカウントの確認</div>');
			return;
              ...
              var object = results[i];
                ...
                group_d = (object.get("group"));
                	if (userclass_d == "管理者" && kengen !== "管理者"){
						$('#er1').html('<div style="color:#EA4335">パスコードが一般ユーザー用の物です。</div>');
						$('#er3').html('');
						$('#my3').html('<div class="blue_btn" onclick="myhelp_tel()">アカウントの確認2</div>');
						return;
                	}
					$('#er1').html('');
					$('#er3').html('');
					...
					$('#my3').append('<div class="blue_btn" tel="'+Tel+'" id="tanmatu1" name="'+userName+'"  pass="'+password+'">変更内容確認</div>');
    ...
    //alert("どうかな"+userName_d+"さん");
					...
					$('#my3').append('<div class="pnk_box">お名前：'+userName_d+'<br>パスワード：-表示できません-<br>クラス：'+userclass_d+'<br>電話番号：'+tel+'</div>');
					$('#my3').append('<div class="blue_btn" id="tanmatu2" name="'+userName_d+'"  pass="'+password_d+'">ログインする</div>');
	...
	//ここまで再登録ログイン
          ...
          if(kekka !== "あった"){
			$('#er1').html('');
			$('#er3').html('<div style="color:#EA4335">入力された電話番号は見つかりませんでした。</div>');
    			ons.notification.alert({title:"エラー",message: '入力された電話番号は見つかりません。'});
			$('#my3').html('<div class="blue_btn" onclick="myhelp_tel()">アカウントの確認3</div>');
          }
          })
         .catch(function(err){
            alert(err);
		    ...
		    .catch(function(err){
    			ons.notification.alert({title:"",message: '電話番号が見つかりません。'});
			$('#my3').html('<div class="blue_btn" onclick="myhelp_tel()">アカウントの確認4</div>');
	...
	ncmb.User.limit(1000)
         ...
         .then(function(results){
          ...
          if(kekka == "あった"){
    			...
    			ons.notification.alert({title:"エラー",message: '入力された電話番号は見つかりませんでした。'});
          }
          })
         .catch(function(err){
            alert(err);
